<div {{ $attributes->merge(['class' => 'user-card w-[342px] h-[342px] p-4 flex-grow-0 shadow-lg opacity-90 border-2 border-primary bg-white', 'data-municipio' => strtolower($user->municipio), 'tabindex' => 0]) }}>
    ...
    <p class="text-semibold m-4">
        <b>Role: </b>
        <span class="{{ $user->role == 'admin' ? 'text-orange' : ($user->role == 'coordinador' ? 'text-primary' : 'text-black') }}">
            {{ $user->role }}
        </span>
    </p>
</div>